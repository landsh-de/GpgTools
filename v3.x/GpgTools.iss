...
;
; - X.509-Certstore updated.
; - Version-Check for x86 AND x64 variant of "gpgol.dll" in order to
;   update to latest version 2.5.7.
; - Fix versioning of .mo installation-sequence for gpgol.dll-update
; - Update Tasks Kill-List (see in code below)
; - Update Version to GpgTools 1.3.26.2 / 1.4.1.2
; - Redesign of functions CheckVerEQ() CheckVerLESS() in order
;   to be file-independent from hardcoded-targets. The target-file is
;   taken from the internal function CurrentFileName(), that may also
;   contain a {code}-reference, so we have to use an ExpandConstant()
;   on this data in order to get the actually processed target-file
;   from the [files]-section during installation.
;
; 20240621
;
...
; - Fix versioning of .mo installation-sequence for gpgol.dll-update
; - GnuPG-Update 2.2.43 (digital-signed) AND GpgOL-Update 2.5.12
;   (not digital-signed) for Gpg4Win 3.1.16.
; - Automatic X.509-Certstore-Updater "BuildTrustList.bat" is now
;   using Mozilla-default branch ROOT-STORE and is also indexing
;   "extra-certs"-store; it supports now downloads behind proxy.
; - Due to false positives on some AV-engines, checked by Virustotal,
;   the AutoIT-tools "StartCon", "MigrateGnuPGKeybase" and
;   "ExpandGNUPGHOME" are now compiled to "a3x"-binaries in order to
;   disable the AutoIT PE-loader usage of AutoIT-exe files. Their
;   execution is now performed by the both "autoit32.exe" and
;   "autoit64.exe" runtime-systems, that were slightly modified in
;   their resources (changed icon and file-description). In order to
;   prevent from a signature-check-error by changed resources, i
;   removed their digital signature from "AutoIt Consulting Ltd",
;   which had expired anyway on 20230804.
; - Update Bass-Lib to 2.4.17.0
;   Fixed an initialization-problem with Bass 2.4.17.x, due to
;   an error i did before, that was not triggered by basslib
;   < 2.4.17. I did not fully defined "BASS_Init flags"
;   (used: 0 here). Now i have to provide a value above "0".
...
; with german resources AFTER installation of "gnupg-w32-update.exe",
; will be retrieved from var: "Gpg4WinVersionB" above ...
; ===================================================================
...
; ## Since "gnupg-vsd"-dir is not a default package-dir in Gpg4Win,##
; ## we're save to delete this stuff (!! be aware !! ... for       ##
; ## GnuPG VS Desktop, this is not the case !!                     ##
; ###################################################################
Type: filesandordirs; Name: "{code:GetGpg4WinInstalled}\share\doc\gnupg-vsd"
...
Source: data\Program Files (x86)\GnuPG\share\doc\gnupg\examples\VS-NfD.prf;       DestDir: {code:GetGnuPG2Installed}\share\doc\gnupg\examples; Flags: ignoreversion uninsrestartdelete overwritereadonly; Components: confpatchtoolpol conftoolpol confpatchtool conftool
...
; ### GPGOL-UPDATE ###
; x86 Install only for gpgol OLDER than gpgol version {#GpgOLVersionB}
Source: data\Program Files (x86)\Gpg4win\bin\gpgol.dll_{#Gpg4WinVersionB}; DestName: "gpgol.dll"; DestDir: {code:GetGpg4WinInstalled}\bin; Flags: ignoreversion uninsneveruninstall overwritereadonly; Components: confpatchtoolpol confpatchtool; Check: CheckVerLESS('{#GpgOLVersionB}')
; x64 Install only for gpgol OLDER than gpgol version {#GpgOLVersionB}
Source: data\Program Files (x86)\Gpg4win\bin_64\gpgol.dll_{#Gpg4WinVersionB}; DestName: "gpgol.dll"; DestDir: {code:GetGpg4WinInstalled}\bin_64; Flags: ignoreversion uninsneveruninstall overwritereadonly; Components: confpatchtoolpol confpatchtool; Check: CheckVerLESS('{#GpgOLVersionB}')
; Install only for gpgol EQUAL gpgol version {#GpgOLVersionB}
Source: data\Program Files (x86)\Gpg4win\share\locale\de\LC_MESSAGES\gpgol.mo_{#Gpg4WinVersionB}; DestName: "gpgol.mo"; DestDir: {code:GetGpg4WinInstalled}\share\locale\de\LC_MESSAGES; Flags: ignoreversion uninsneveruninstall overwritereadonly; Components: confpatchtoolpol confpatchtool; Check: CheckGpgOLVerEQ('{#GpgOLVersionB}')
; Install only for gpgol EQUAL gpgol version {#GpgOLVersion}
Source: data\Program Files (x86)\Gpg4win\share\locale\de\LC_MESSAGES\gpgol.mo_{#Gpg4WinVersion}; DestName: "gpgol.mo"; DestDir: {code:GetGpg4WinInstalled}\share\locale\de\LC_MESSAGES; Flags: ignoreversion uninsneveruninstall overwritereadonly; Components: confpatchtoolpol confpatchtool; Check: CheckGpgOLVerEQ('{#GpgOLVersion}')
; Install only for Gpg4Win Var:"Gpg4WinVersion"
...
Source: data\Program Files (x86)\Gpg4win\share\doc\gnupg-vsd\*; DestDir: {code:GetGpg4WinInstalled}\share\doc\gnupg-vsd; Flags: ignoreversion recursesubdirs createallsubdirs uninsrestartdelete overwritereadonly; Components: confpatchtoolpol conftoolpol confpatchtool conftool
; ########################## Program-Icon ###########################
...
Source: data\Program Files (x86)\GpgTools\Icon4.ico; DestDir: {app}; Flags: ignoreversion uninsrestartdelete overwritereadonly; Components: confpatchtoolpol conftoolpol confpatchtool conftool
Source: data\Program Files (x86)\GpgTools\Icon5.ico; DestDir: {app}; Flags: ignoreversion uninsrestartdelete overwritereadonly; Components: confpatchtoolpol conftoolpol confpatchtool conftool
Source: data\Program Files (x86)\GpgTools\Icon6.ico; DestDir: {app}; Flags: ignoreversion uninsrestartdelete overwritereadonly; Components: confpatchtoolpol conftoolpol confpatchtool conftool
Source: data\Program Files (x86)\GpgTools\Icon7.ico; DestDir: {app}; Flags: ignoreversion uninsrestartdelete overwritereadonly; Components: confpatchtoolpol conftoolpol confpatchtool conftool
Source: data\Program Files (x86)\GpgTools\autoit32.exe; DestDir: {app}; Flags: ignoreversion uninsrestartdelete overwritereadonly; Components: confpatchtoolpol conftoolpol confpatchtool conftool
Source: data\Program Files (x86)\GpgTools\autoit64.exe; DestDir: {app}; Flags: ignoreversion uninsrestartdelete overwritereadonly; Components: confpatchtoolpol conftoolpol confpatchtool conftool
; ########################## Program-Tools ##########################
...
Root: "HKLM"; Subkey: "Software\Microsoft\Windows\CurrentVersion\Run"; ValueType: expandsz; ValueName: "Gpg4Win_Cleanup"; ValueData: """{app}\autoit32.exe"" ""{app}\StartCon\Gpg4WinPreConfig.a3x"""; Flags: uninsdeletevalue; Components: confpatchtoolpol conftoolpol confpatchtool conftool
...
; Policy Execution Enabler GpgTools -autoit32-
Root: "HKLM"; Subkey: "Software\Policies\Microsoft\Windows\Safer\CodeIdentifiers\262144\Paths\{{AD1BA620-A9CE-46f0-9F42-BE0403AE7124}"; ValueType: string; ValueName: "Description"; ValueData: "AutoIT32 Runner ( autoit32.exe ) ..."; Flags: uninsdeletekeyifempty uninsdeletevalue; Components: confpatchtoolpol conftoolpol; Check: CheckSRPIsEnabled()
Root: "HKLM"; Subkey: "Software\Policies\Microsoft\Windows\Safer\CodeIdentifiers\262144\Paths\{{AD1BA620-A9CE-46f0-9F42-BE0403AE7124}"; ValueType: string; ValueName: "ItemData"; ValueData: "{app}\autoit32.exe"; Flags: uninsdeletekeyifempty uninsdeletevalue; Components: confpatchtoolpol conftoolpol; Check: CheckSRPIsEnabled()
...
; Policy Execution Enabler GpgTools -autoit64-
Root: "HKLM"; Subkey: "Software\Policies\Microsoft\Windows\Safer\CodeIdentifiers\262144\Paths\{{6333CE0C-BCF2-4dee-AAEB-E85CAFACA1AD}"; ValueType: string; ValueName: "Description"; ValueData: "AutoIT64 Runner ( autoit64.exe ) ..."; Flags: uninsdeletekeyifempty uninsdeletevalue; Components: confpatchtoolpol conftoolpol; Check: CheckSRPIsEnabled()
Root: "HKLM"; Subkey: "Software\Policies\Microsoft\Windows\Safer\CodeIdentifiers\262144\Paths\{{6333CE0C-BCF2-4dee-AAEB-E85CAFACA1AD}"; ValueType: string; ValueName: "ItemData"; ValueData: "{app}\autoit64.exe"; Flags: uninsdeletekeyifempty uninsdeletevalue; Components: confpatchtoolpol conftoolpol; Check: CheckSRPIsEnabled()
...
Root: "HKLM"; Subkey: "Software\Policies\Microsoft\Windows\SrpV2\Exe\6C643BE4-2CCA-4e60-BC6B-48C43C044224"; ValueType: string; ValueName: "Value"; ValueData: "<FilePathRule Id=""6C643BE4-2CCA-4e60-BC6B-48C43C044224"" Name=""GpgTools_AutoITRunner32"" Description=""(GpgTools) Allows execution of &quot;autoit32.exe&quot;"" UserOrGroupSid=""S-1-1-0"" Action=""Allow""><Conditions><FilePathCondition Path=""{app}\autoit32.exe""/></Conditions></FilePathRule>"; Flags: uninsdeletekeyifempty uninsdeletevalue; Components: confpatchtoolpol conftoolpol; Check: EnforcementModeEXEActive()
Root: "HKLM"; Subkey: "Software\Policies\Microsoft\Windows\SrpV2\Exe\0EDFEB20-0262-430d-98DF-41C459967D04"; ValueType: string; ValueName: "Value"; ValueData: "<FilePathRule Id=""0EDFEB20-0262-430d-98DF-41C459967D04"" Name=""GpgTools_AutoITRunner64"" Description=""(GpgTools) Allows execution of &quot;autoit64.exe&quot;"" UserOrGroupSid=""S-1-1-0"" Action=""Allow""><Conditions><FilePathCondition Path=""{app}\autoit64.exe""/></Conditions></FilePathRule>"; Flags: uninsdeletekeyifempty uninsdeletevalue; Components: confpatchtoolpol conftoolpol; Check: EnforcementModeEXEActive()
...
; Icons GnuPG
; Note:
; When Setup is running on 64-bit Windows, it will automatically replace
; {commonpf32}\'s value in the filename with '%ProgramFiles(x86)%\' to work
; around a bug in 64-bit Windows: 64-bit Windows replaces it with
; '%ProgramFiles%\' instead which is incorrect.
...
; Tasks kill-list for process-control before installation of files,
...
var
  ...
  //
  // Date: 01/2024
  // BASS updated to version v2.4.17.0
  //
...
const
  ...
  //BASSVERSION = $02041007;    // (decimal 33820416) API version 2.4.16.7 (from 08.09.2021)
  //BASSVERSION = $02041100;    // (decimal 33820928) API version 2.4.17.0 (from 12.10.2022)
  BASSVERSION = $02041100;      // (decimal 33820928) API version 2.4.17.0 (from 12.10.2022)
...
//////////////////////////////////////////////////////////////////////////////
// https://stackoverflow.com/questions/37825650/compare-version-strings-in-inno-setup/37826864#37826864
function CompareVersion(V1, V2: string): Integer;
// This function compares strings; the function "ComparePackedVersion" uses
// integers. Inno Setup 6.1 and newer supports version comparing natively with
// its function "ComparePackedVersion":
// https://jrsoftware.org/ishelp/index.php?topic=isxfunc_comparepackedversion
// For "ComparePackedVersion" function use "GetPackedVersion" to obtain binary
// version in the required form. And "PackVersionComponents" or "StrToVersion"
// to calculate the packed version for a specific (hardcoded) version for the
// comparison.
...
//              Throws an exception, when a version is syntactically invalid
...
//////////////////////////////////////////////////////////////////////////////
function CheckVerLESS (MyVer: String): Boolean;
// Checks if installed version IS LESS than the provided version

var
  MyTargetVERSION: String;
  MyTargetNAME: String;
  // MyTargetPATH: String;

begin
    // Example for ExpandConstant(CurrentFileName):
    // "C:\Program Files (x86)\Gpg4win\bin\gpgol.dll"
    MyTargetNAME := ExpandConstant(CurrentFileName);
    // Example for ExpandConstant(ExtractFilePath(CurrentFileName)):
    // "C:\Program Files (x86)\Gpg4win\bin\"
    // MyTargetPATH := ExpandConstant(ExtractFilePath(CurrentFileName));

    if (FileExists(MyTargetNAME)) then begin
       Log('CheckVerLESS(): File: ' + MyTargetNAME + ' does exist.');
       if (GetVersionNumbersString(MyTargetNAME, MyTargetVERSION)) then begin
          Log('CheckVerLESS(): File Version-Number: ' + '"' + MyTargetVERSION + '"');
          Log('CheckVerLESS(): Provided Version-Number to compare: ' + '"' + MyVer + '"');
          // Result: 0 if equal, -1 if V1 older V2, 1 if V1 newer V2.
          // Update if version is newer than version of A OR than version of B
         	if ((CompareVersion(MyVer,MyTargetVERSION)) = 1) then begin
             Log('CheckVerLESS(): ' + MyVer + ' IS NEWER THAN ' + MyTargetVERSION + '.');
             Log('CheckVerLESS(): *** FILE WILL BE UPDATED ! ***');
             Result := True;
          end else begin
             Log('CheckVerLESS(): ' + MyVer + ' OLDER THAN OR EQUAL TO ' + MyTargetVERSION + '.');
             Log('CheckVerLESS(): *** FILE WILL NOT BE UPDATED ! ***');
             Result := False;
          end;
       end else begin
           Log('CheckVerLESS(): Something went wrong in getting Version-Number: ' + '"' +MyTargetVERSION + '"');
           Result := False;
       end;
    end else begin
       Log('CheckVerLESS(): File does not exist: ' + '"' + MyTargetNAME + '" !');
       Result := False;
    end;
end;
...
//////////////////////////////////////////////////////////////////////////////
function CheckVerEQ (MyVer: String): Boolean;
// Checks if installed version IS EQUAL to the provided version
...
begin
    ...
    if (FileExists(MyTargetNAME)) then begin
       Log('CheckVerEQ(): File: ' + MyTargetNAME + ' does exist.');
       if (GetVersionNumbersString(MyTargetNAME, MyTargetVERSION)) then begin
          Log('CheckVerEQ(): File Version-Number: ' + '"' + MyTargetVERSION + '"');
          Log('CheckVerEQ(): Provided Version-Number to compare: ' + '"' + MyVer + '"');
          // Result: 0 if equal, -1 if V1 older V2, 1 if V1 newer V2.
          // Update if version is newer than version of A OR than version of B
         	if ((CompareVersion(MyVer,MyTargetVERSION)) = 0) then begin
             Log('CheckVerEQ(): ' + MyVer + ' IS EQUAL TO ' + MyTargetVERSION + '.');
             Log('CheckVerEQ(): *** FILE WILL BE UPDATED ! ***');
             Result := True;
          end else begin
             Log('CheckVerEQ(): ' + MyVer + ' IS NOT EQUAL TO ' + MyTargetVERSION + '.');
             Log('CheckVerEQ(): *** FILE WILL NOT BE UPDATED ! ***');
             Result := False;
          end;
       end else begin
           Log('CheckVerEQ(): Something went wrong in getting Version-Number: ' + '"' +MyTargetVERSION + '"');
           Result := False;
       end;
    end else begin
       Log('CheckVerEQ(): File does not exist: ' + '"' + MyTargetNAME + '" !');
       Result := False;
    end;
end;

//////////////////////////////////////////////////////////////////////////////
function CheckGpgOLVerEQ (MyVer: String): Boolean;
// Checks if installed version IS EQUAL to the provided version

var
  GpgOl_Version: String;
  GpgOl_Version64: String;
  Install_Directory_Gpg4win: String;
  aExecFile: String;
  aExecFile64: String;
...
begin
    if (RegQueryStringValue(HKEY_LOCAL_MACHINE, 'Software\WOW6432Node\Gpg4win',
        'Install Directory', Install_Directory_Gpg4win)) AND (Pos('.', UpperCase(MyVer)) > 0) then begin

        // Value was read successfully
        Log('CheckGpgOLVerEQ(): Install Directory of Gpg4win exist in Registry: ' + Install_Directory_Gpg4win);
        aExecFile := Install_Directory_Gpg4win + '\bin\gpgol.dll';
        aExecFile64 := Install_Directory_Gpg4win + '\bin_64\gpgol.dll';

        if (FileExists(aExecFile)) AND (FileExists(aExecFile64)) then begin
           ...
           if (GetVersionNumbersString(aExecFile, GpgOl_Version)) AND (GetVersionNumbersString(aExecFile64, GpgOl_Version64)) then begin
              ...
              // Update if version is equal to version of A AND to version of B
                 ...
                 Log('CheckGpgOLVerEQ(): ' + MyVer + ' IS NOT EQUAL TO ' + GpgOl_Version + ' AND ' + GpgOl_Version64 + '.');
                 Log('CheckGpgOLVerEQ(): *** FILE WILL NOT BE UPDATED ! ***');
                 Result := False;
              end;
           end else begin
              Log('CheckGpgOLVerEQ(): Something went wrong in getting Version-Number: ' + '"' + GpgOl_Version + '"' + ' AND "' + GpgOl_Version64 + '"');
              Result := False;
           end;
        end else begin
           Log('CheckGpgOLVerEQ(): File does not exist: ' + '"' + aExecFile + '" ...');
           Log('CheckGpgOLVerEQ(): AND file does not exist: ' + '"' + aExecFile64 + '"');
           Result := False;
        end;
    end else begin
        // Value was NOT read successfully
        Log('CheckGpgOLVerEQ(): Install Directory of Gpg4win does not exist in Registry: ' + Install_Directory_Gpg4win);
        Log('CheckGpgOLVerEQ(): ... or "." was missing in Version-String.');
        Result := False;
    end;
end;
...
begin
  ...
  if (SkipSound() = False) AND (FileExists(SndName) = True) then begin
    ...
    BASS_Version_Result := BASS_GetVersion;
      ...
      // an error i did before, that was not triggered by basslib
      // < 2.4.17. I did not fully defined "BASS_Init flags"
      // (used: 0 here). Now we have to provide a value above "0".
      // For "DirectSound", we must use "BASS_DEVICE_DSOUND" as
      // value "$40000"; "0" is NOT DEFINED here !!
      BASS_Init_Result := BASS_Init(-1, 44100, BASS_DEVICE_DSOUND, 0, 0);
  ...
  // ---  END  ---
    ...
    // ###########################################################################
    // !! Kill processes in order to update locked files                        !!
    // ###########################################################################
